...
def build_theory(series, m):
    ...
    if series == 'A':
        ...
        def chi_fold(outer, inner, tau):
            """Virasoro character with the *folded* index = inner and the
            spectator index = outer (folding acts on the even index N_even)."""
            if side == 'q':
                return character(p, q, outer, inner, tau)
            return character(p, q, inner, outer, tau)

        # The D-series is generated by the order-2 simple current phi_{1,q-1}
        # (equivalently phi_{p-1,1}), whose spin is  h_J = (N_even-2)(N_even-3)/4
        # for the folded factor.  There are TWO structurally different invariants:
        #
        #   * N_even = 2 (mod 4)  ->  half odd  ->  h_J integer  ->  the current
        #     extends the chiral algebra: block-diagonal (D_even) invariant,
        #     Z = 1/2 sum |chi_a + chi_{N-a}|^2 over odd a, with a |chi_{N/2}|^2
        #     fixed-point term.
        #
        #   * N_even = 0 (mod 4)  ->  half even ->  h_J half-integer  ->  NO bosonic
        #     extension exists; the invariant is the charge-conjugation AUTOMORPHISM
        #     (D_odd), Z = 1/2 sum_{a odd}|chi_a|^2 + 1/2 sum_{a even} chi_a conj(chi_{N-a}).
        #
        # Using the block form for the N_even = 0 (mod 4) case (as a naive
        # even/odd guard would) yields a combination that is S-invariant but NOT
        # T-invariant -- i.e. not a genuine torus partition function.
        if half % 2 == 1:                         # D_even: block-diagonal extension
            pairs = [(a, N_even - a) for a in range(1, half, 2)]   # odd a < N_even/2
            ...
            def Z_target(tau):
                ...
                for outer in outer_range:
                    acc = 0.0
                    for a, b in pairs:
                        acc += abs(chi_fold(outer, a, tau)
                                   + chi_fold(outer, b, tau)) ** 2
                    acc += 2.0 * abs(chi_fold(outer, half, tau)) ** 2  # fixed point a=N/2
                    total += 0.5 * acc
                return float(total)

            d_type = 'D_even'
        else:                                     # D_odd: conjugation automorphism
            def Z_target(tau):
                total = 0.0
                for outer in outer_range:
                    acc = 0.0
                    for a in range(1, N_even, 2):            # odd a: diagonal |chi|^2
                        acc += abs(chi_fold(outer, a, tau)) ** 2
                    for a in range(2, N_even, 2):            # even a: chi_a conj(chi_{N-a})
                        acc += (chi_fold(outer, a, tau)
                                * chi_fold(outer, N_even - a, tau).conjugate()).real
                    total += 0.5 * acc
                return float(total)

            d_type = 'D_odd'

        series_label = f'D{D_index} ({d_type}, {side}-side)  M({p},{q})'
