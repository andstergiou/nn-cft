#!/usr/bin/env python3
"""
Neural network bootstrap for the 3D Ising epsilon four-point function.

Trains an ensemble of neural networks to learn the four-point function G(z)
of the epsilon operator (Delta_epsilon = 1.412625) in the 3D Ising CFT by
enforcing the crossing symmetry constraint

    G(z) = (z / (1-z))^(2*Delta_epsilon) * G(1-z).
...
Usage (personal computer):
    ...
    python run_3d_ising_epsilon.py --num-runs 10 --output-dir results_epsilon
    python run_3d_ising_epsilon.py --num-runs 3 --anchor-points 0.2 0.4
...
# Default conformal dimension for the 3D Ising epsilon operator
DELTA_EPSILON = 1.412625

# ---------------------------------------------------------------------------
# Bootstrap data: (spin n, dimension Delta, OPE coefficient squared a)
# Operators appearing in the epsilon x epsilon OPE of the 3D Ising CFT.
# Data from Simmons-Duffin (2016), arXiv:1612.08471.
# ---------------------------------------------------------------------------
DATA_BOOTSTRAP = [
    (0,       1.412625,  2.34835702922),
    (0,       3.82968,   2.359296),
    (0,       6.8956,    0.01635841),
    (0,       7.2535,    0.03511876),
    (2 // 2,  3,         2.108220174502426),
    (2 // 2,  5.50915,   1.27044654106666),
    (2 // 2,  7.0758,    0.127685846),
    (4 // 2,  5.02267,   0.224783822262857),
    (4 // 2,  6.42065,   0.0444503808329142),
    (4 // 2,  7.38568,   0.193042514285714),
    (6 // 2,  7.02849,   0.0193893757052121),
    (8 // 2,  9.03102,   0.00152720330182315),
    (10 // 2, 11.0324,   0.00011396477399506),
    (12 // 2, 13.0333,   8.22994730079595e-6),
    (6 // 2,  8.4957,    0.0082346001731601),
    (8 // 2,  10.562,    0.00098385474436674),
    (10 // 2, 12.5659,   0.000111732680974604),
    (12 // 2, 14.633,    9.29503414670455e-6),
]
...
class HNet(nn.Module):
    ...
    def forward(self, z):
        return z ** self.delta * (1 - z) ** (-2 * self.delta) * self.net(z)
...
def plot_results(z_values, bootstrap_values, all_predictions, output_dir, delta):
    ...
    fig.suptitle(f"3D Ising epsilon four-point function  (Delta_epsilon={delta})", fontsize=13)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train an ensemble of networks for the 3D Ising epsilon four-point function."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir", type=str, default="results_3d_epsilon",
        help="Directory for results and plots (default: results_3d_epsilon).",
    )
    parser.add_argument(
        "--delta", type=str, default="1.412625",
        help=f"Conformal dimension of epsilon (default: {DELTA_EPSILON}, the 3D Ising value).",
    )
    parser.add_argument(
    ...
    print("=" * 60)
    print("3D Ising Epsilon Neural Network Bootstrap")
    print(f"  delta_epsilon : {args.delta} = {delta:.6f}")
